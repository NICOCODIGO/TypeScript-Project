// what typscript does is that it will not allow you to make mistake since it will check the types of the variables
...
interface Threat { //strings means that it is not optional, because in the threatsdata all the properties have something in common, like in logs, majority of the text inside are quoted
    id: string;
    threatName: string;
    logs: string;
    severity: string;
    affectedSystems?: string | string[]; // ? means that it is optional, because in the threatsdata some of the properties have something in common, like in affectedSystems, some are arrays (some have this symbol []) and some are just strings (ones with quotes )
    status: string;
    dateResolved?: string; // in the data.ts some  data have dateResolved and some don't
} //the point of this interface is to help write better code below, so that we can use it later in the code

function addRow(table: HTMLTableElement, threat : Threat) { // function to add a row to the table, in list format 
...
function selectTable() {
    return <HTMLTableElement>document.querySelector("#SecurityTable"); // type assertion to tell TS that this is an HTMLTableElement
}

function refreshTable(table: HTMLTableElement, threats: Threat[]) {
